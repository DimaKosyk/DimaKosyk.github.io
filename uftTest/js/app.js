(() => {
    ...
    function initSliders() {
        let rationSwiper;
        if (window.innerWidth <= 768) rationSwiper = new swiper_core_Swiper(".ration-tabs__slider", {
            modules: [ Scrollbar ],
            speed: 5e3,
            slidesPerView: 1.4,
            spaceBetween: 24,
            scrollbar: {
                el: ".ration-tabs-scrollbar",
                draggable: true,
                dragSize: 20
            }
        }); else if (rationSwiper) rationSwiper.destroy(true, true);
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (scrollableContent && scrollThumb) scrollableContent.addEventListener("scroll", (() => {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        const cursorText = document.querySelector(".my-cursor-text");
        const reviewSlider = document.querySelector(".reviews__slider");
        let isDragging = false;
        if (myCursor && cursorText && reviewSlider) {
            const texts = {
                ru: "Тягни",
                ua: "Тягни"
            };
            function setLanguage(lang) {
                cursorText.textContent = texts[lang];
            }
            ...
            reviewSlider.addEventListener("mouseenter", (() => {
                myCursor.classList.add("cursor-active");
            }));
            reviewSlider.addEventListener("mouseleave", (() => {
                myCursor.classList.remove("cursor-active");
            }));
            setLanguage("ua");
        }
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const bonusButton = document.querySelector(".order-days-tabs__bonus");
        const bonusHover = document.querySelector(".bonus-hover-order");
        const closeButton = document.querySelector(".bonus-hover-order__close");
        if (bonusButton && bonusHover && closeButton) {
            bonusButton.addEventListener("click", (() => {
                bonusHover.classList.add("bonus-active");
            }));
            closeButton.addEventListener("click", (event => {
                event.stopPropagation();
                bonusHover.classList.remove("bonus-active");
            }));
        }
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const shareButtons = document.querySelectorAll("[data-link]");
        shareButtons.forEach((button => {
            button.addEventListener("click", (() => {
                const content = button.dataset.link?.trim();
                navigator.clipboard.writeText(content).then((r => {
                    button.classList.add("copied");
                    setTimeout((() => {
                        button.classList.remove("copied");
                    }), 3e3);
                })).catch((error => console.log("От халепа... " + error, "error")));
            }));
        }));
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const phoneInputs = document.querySelectorAll('[type="tel"], .phone');
        [].forEach.call(phoneInputs, (function(input) {
            var keyCode;
            function mask(event) {
                event.keyCode && (keyCode = event.keyCode);
                this.selectionStart;
                var matrix = `+38 (0__) __-__-___`, i = 0, def = matrix.replace(/\D/g, ""), val = this.value.replace(/\D/g, ""), new_value = matrix.replace(/[_\d]/g, (function(a) {
                ...
                reg = new RegExp("^" + reg + "$");
                if (!reg.test(this.value) || this.value.length < num || keyCode > 47 && keyCode < 58) this.value = new_value;
    ...
    function prevStep(form) {
        const currentStep = form.dataset.step ? parseInt(form.dataset.step) : 0;
        if (currentStep > 0) {
            const newStep = currentStep - 1;
            form.dataset.step = newStep;
            showStep(newStep, form);
        }
    }
    document.addEventListener("DOMContentLoaded", (() => {
        document.querySelectorAll(".multi-step-form").forEach((form => {
            form.dataset.step = 0;
            form.querySelectorAll('button[data-action="next"]').forEach((button => {
                button.addEventListener("click", (() => {
                    const currentStep = form.dataset.step ? parseInt(form.dataset.step) : 0;
                    const steps = form.querySelectorAll(".step");
            ...
            form.querySelectorAll('button[data-action="prev"]').forEach((button => {
                button.addEventListener("click", (() => prevStep(form)));
            }));
    ...
    videoContainers.forEach((container => {
        ...
        const youtubeVideo = container.querySelector(".youtube-video");
        if (playButton && videoPreview && videoElement) playButton.addEventListener("click", (function() {
            ...
            document.addEventListener("click", (function(event) {
                if (!videoElement.contains(event.target) && !playButton.contains(event.target)) {
        ...
        if (playButton && videoPreview && videoWrapper && youtubeVideo) playButton.addEventListener("click", (function() {
            const videoContainer = container.querySelector(".video-reviews__video-wrapper");
            ...
            document.addEventListener("click", (function(event) {
                if (!videoWrapper.contains(event.target) && !playButton.contains(event.target)) {
                    youtubeVideo.src = "";
