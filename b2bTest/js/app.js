(() => {
    var __webpack_modules__ = {
        210: function(__unused_webpack_module, exports) {
    ...
    (() => {
        ...
        const esm = flatpickr;
        var uk = __webpack_require__(210);
        document.addEventListener("DOMContentLoaded", () => {
            const aside = document.querySelector(".aside-navbar");
            const toggleBtn = document.querySelector(".open-aside");
            const body = document.body;
            if (aside && toggleBtn) toggleBtn.addEventListener("click", () => {
                aside.classList.toggle("open");
                if (window.innerWidth < 992) body.classList.toggle("lock", aside.classList.contains("open"));
            });
        });
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".product-quantity").forEach(selector => {
                ...
                const maxValue = maxAttr ? parseInt(maxAttr) : null;
                ...
                function updateButtons() {
                    ...
                    if (currentValue === null) {
                        decreaseBtn.classList.add("min-value");
                        increaseBtn.classList.remove("max-value");
                        return;
                    }
                    if (currentValue <= minValue) decreaseBtn.classList.add("min-value"); else decreaseBtn.classList.remove("min-value");
                    if (maxValue !== null && currentValue >= maxValue) increaseBtn.classList.add("max-value"); else increaseBtn.classList.remove("max-value");
                }
                ...
                increaseBtn.addEventListener("click", function() {
                    let value = getCurrentValue();
                    if (value === null) quantityInput.value = minValue; else if (maxValue === null || value < maxValue) quantityInput.value = value + 1;
                    updateButtons();
                });
                quantityInput.addEventListener("input", function() {
                    let raw = quantityInput.value.replace(/\D/g, "");
                    raw = raw.replace(/^0+/, "");
                    if (raw === "") {
                        quantityInput.value = "";
                        updateButtons();
                ...
                quantityInput.addEventListener("blur", function() {
                    let value = getCurrentValue();
                    if (value === null) quantityInput.value = ""; else {
                        if (value < minValue) quantityInput.value = minValue;
                        if (maxValue !== null && value > maxValue) quantityInput.value = maxValue;
                    }
                    updateButtons();
                });
                updateButtons();
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            dropDowns.forEach(dropDown => {
                ...
                const dropDownEm = dropDownButton.querySelector("em");
                ...
                items.forEach(li => {
                    li.addEventListener("click", function() {
                        const clickedBtn = li.querySelector("button");
                        if (clickedBtn) {
                            const newText = clickedBtn.textContent;
                            const currentActive = dropDown.querySelector("ul li span");
                            if (currentActive) {
                                const oldText = currentActive.textContent;
                                const newButton = document.createElement("button");
                                newButton.type = "button";
                                newButton.textContent = oldText;
                                currentActive.replaceWith(newButton);
                            }
                            const newSpan = document.createElement("span");
                            newSpan.textContent = newText;
                            clickedBtn.replaceWith(newSpan);
                            dropDownSpan.textContent = newText;
                            if (dropDownEm) dropDownEm.remove();
        ...
        document.addEventListener("DOMContentLoaded", () => {
            const gridBtn = document.querySelector(".grid-version-button");
            const listBtn = document.querySelector(".list-version-button");
            const gridContent = document.querySelector(".catalogue-content-grid");
            const listContent = document.querySelector(".catalogue-content-list");
        ...
        document.addEventListener("DOMContentLoaded", () => {
            const buttons = document.querySelectorAll(".show-additional-info");
            buttons.forEach(button => {
                button.addEventListener("click", () => {
                    const orderMain = button.closest(".order-main");
                    const additional = orderMain.querySelector(".order-additional");
                    if (orderMain.classList.contains("active")) {
                        additional.style.height = additional.scrollHeight + "px";
                        requestAnimationFrame(() => {
                            additional.style.height = "0";
                        });
                        orderMain.classList.remove("active");
                    } else {
                        additional.style.height = additional.scrollHeight + "px";
                        additional.addEventListener("transitionend", function handler() {
                            additional.style.height = "auto";
                            additional.removeEventListener("transitionend", handler);
                        });
                        orderMain.classList.add("active");
                    }
                });
            });
        });
        esm("#calendar", {
            ...
            mode: "range",
            prevArrow: `\n\t\t\t\t<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">\n\t\t\t\t\t<path d="M12.5 15L7.5 10L12.5 5" stroke="#B2B6C0" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>\n\t\t\t\t</svg>\n\t\t\t`,
            nextArrow: `\n\t\t\t\t<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">\n\t\t\t\t\t<path d="M7.5 15L12.5 10L7.5 5" stroke="#B2B6C0" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>\n\t\t\t\t</svg>\n\t\t\t`
        });
        document.addEventListener("DOMContentLoaded", () => {
            const downloadDocs = document.querySelectorAll(".download-document");
            downloadDocs.forEach(downloadDoc => {
                const toggleBtn = downloadDoc.querySelector(".btn-blue");
                toggleBtn.addEventListener("click", e => {
                    e.stopPropagation();
                    downloadDocs.forEach(doc => {
                        if (doc !== downloadDoc) doc.classList.remove("active");
                    });
                    downloadDoc.classList.toggle("active");
                });
            });
            document.addEventListener("click", e => {
                downloadDocs.forEach(doc => {
                    if (!doc.contains(e.target)) doc.classList.remove("active");
                });
            });
        });
        document.addEventListener("DOMContentLoaded", () => {
            const openModalBtns = document.querySelectorAll("[data-target-modal]");
            ...
            function closeModal(event) {
                ...
                if (!modal) return;
                if (event.target.closest(".js-close-btn") || event.target.classList.contains("modal") && modal.classList.contains("active")) closeModalAnimation();
            }
            ...
            openModalBtns.forEach(btn => {
                btn.addEventListener("click", e => {
                    e.preventDefault();
                    const targetModal = btn.getAttribute("data-target-modal");
        ...
        function validatePhone(input) {
            let regex = new RegExp(`^\\+38 \\(\\d{3}\\) \\d{2}-\\d{2}-\\d{3}$`);
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function onPhoneBlur(e) {
                const regex = /^\+38 \(\d{3}\) \d{2}-\d{2}-\d{3}$/;
        ...
        document.querySelectorAll(".password-wrapper").forEach(wrapper => {
            ...
            const eyeClosedSVG = `\n\t\t<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">\n\t\t<g clip-path="url(#clip0_458_431)">\n\t\t\t<path d="M17.94 17.94C16.2306 19.243 14.1491 19.9649 12 20C5 20 1 12 1 12C2.24389 9.68192 3.96914 7.65663 6.06 6.06003M9.9 4.24002C10.5883 4.0789 11.2931 3.99836 12 4.00003C19 4.00003 23 12 23 12C22.393 13.1356 21.6691 14.2048 20.84 15.19M14.12 14.12C13.8454 14.4148 13.5141 14.6512 13.1462 14.8151C12.7782 14.9791 12.3809 15.0673 11.9781 15.0744C11.5753 15.0815 11.1752 15.0074 10.8016 14.8565C10.4281 14.7056 10.0887 14.4811 9.80385 14.1962C9.51897 13.9113 9.29439 13.572 9.14351 13.1984C8.99262 12.8249 8.91853 12.4247 8.92563 12.0219C8.93274 11.6191 9.02091 11.2219 9.18488 10.8539C9.34884 10.4859 9.58525 10.1547 9.88 9.88003" stroke="#B2B6C0" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />\n\t\t\t<path d="M1 1L23 23" stroke="#B2B6C0" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />\n\t\t</g>\n\t\t<defs>\n\t\t\t<clipPath id="clip0_458_431">\n\t\t\t\t<rect width="24" height="24" fill="white" />\n\t\t\t</clipPath>\n\t\t</defs>\n\t</svg>`;
            const eyeOpenSVG = `\n\t\t<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">\n\t\t\t<path d="M1 12C1 12 5 4 12 4C19 4 23 12 23 12C23 12 19 20 12 20C5 20 1 12 1 12Z" stroke="#B2B6C0" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>\n\t\t\t<path d="M12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15Z" stroke="#B2B6C0" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>\n\t\t</svg>`;
        ...
        document.addEventListener("DOMContentLoaded", function() {
            const openCatalogBtn = document.querySelector(".open-catalog-mobile");
            const catalogMobile = document.querySelector(".catalogue-menu");
            const closeCatalogueBtn = document.querySelector(".close-catalogue-button");
            if (openCatalogBtn && catalogMobile && closeCatalogueBtn) {
                openCatalogBtn.addEventListener("click", function() {
                    catalogMobile.classList.add("active");
                    document.body.classList.add("lock");
                });
                closeCatalogueBtn.addEventListener("click", function() {
                    catalogMobile.classList.remove("active");
                    document.body.classList.remove("lock");
                    document.querySelectorAll(".sub-catalog-menu.active").forEach(el => el.classList.remove("active"));
                });
            }
            const catalogButtons = document.querySelectorAll("[data-id]");
            catalogButtons.forEach(button => {
                button.addEventListener("click", function() {
                    const targetSelector = this.getAttribute("data-id");
                    const targetElement = document.querySelector(targetSelector);
                    if (targetElement) targetElement.classList.add("active");
                });
            });
            const backToCatalogButtons = document.querySelectorAll(".back-to-catalog-button");
            backToCatalogButtons.forEach(button => {
                button.addEventListener("click", function() {
                    const parentSubCatalog = this.closest(".sub-catalog-menu");
                    if (parentSubCatalog) parentSubCatalog.classList.remove("active");
                });
            });
        });
        window["FLS"] = false;
